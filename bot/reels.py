...
class ReelsManager:
    ...
    def send_reel(self, reels_id: str, loop):
        ...
        asyncio.run_coroutine_threadsafe(
            reel.channel.send(
                f":warning:{reel.channel.name} time:warning:\nSend a pic and/or message of what you're up to!!!\n@everyone",
                allowed_mentions=mention_all,
            ),
            loop,
        )
    ...
    def _trigger_send_timer(self, reels_id: str, loop=None, now=False):
        ...
        if now:
            ...
            trigger_date = (current_dt + timedelta(days=next_day)).replace(
        ...
        logger.info(
            f"Next reel for {reels_id} is at {reel.last_reel + timedelta(seconds=new_timeout)}"
        )
        loop = asyncio.get_event_loop() if loop is None else loop
        t = Timer(
            new_timeout, self.send_reel, kwargs={"reels_id": reels_id, "loop": loop}
        )
        t.setDaemon(True)
        t.start()
