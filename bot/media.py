...
class BotAudioSource:
    def __init__(self, source_str: str, name: str, duration: str):
        ...
        self.source: discord.PCMVolumeTransformer = discord.PCMVolumeTransformer(
            discord.FFmpegPCMAudio(source_str)
        )
...
class AudioContext:
    def __init__(self):
        ...
        self._now_playing: Optional[BotAudioSource] = None
    ...
    async def disconnect_channel(self, force=False):
        if not self._vc:
            logger.error("Can't disconnect if never joined channel before")
            raise NoVoiceClientError

        if force or self._vc.is_connected():
            if force:
                logger.info("Forcing disconnect")
            self._vc.cleanup()
            await self._vc.disconnect(force=force)
        else:
            logger.info("Can't disconnect if not in channel")
    ...
    def done_playing(self, e):
        ...
        if self._repeat_flag and not self._stop_flag and not self._skip_flag:
            ...
            bas: BotAudioSource = BotAudioSource(
                self._now_playing.source_str,
                self._now_playing.name,
                self._now_playing.duration,
            )
